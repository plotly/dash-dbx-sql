...
app.layout = dmc.MantineProvider(
    ...
    children=dmc.NotificationsProvider([
        ...
        dmc.Affix(
            html.A("See code", href="https://github.com/plotly/dash-dbx-sql", target="_blank", className="demo-button"), 
...
def make_userpage(userid, fitness):
    ...
    blood_pressure = dmc.Text(f"Blood Pressure Level: {df_userdemo['bloodpressure'][0]}")
    chorestelor = dmc.Text(f"Cholesterol Level: {df_userdemo['cholesterol'][0]}")
    patient_info = dmc.Text(f"Patient is a {df_userdemo['age'][0]} old {df_userdemo['sex'][0].lower()}, weights {df_userdemo['weight'][0]} lbs, and is a {df_userdemo['Smoker'][0].lower()}")
    ...
    return [patient_info, chorestelor], [user_comparison, blood_pressure], header, fig_user, comp.notification_user(notification)
...
def make_heatmap(axes, fitness, comparison, slider):
    if len(axes) == 2:
        ...
        notification, action = f"Scatter data loaded. \n1 query executed with number of rows retrieved: {len(df_heat)}", "show"
    else:
